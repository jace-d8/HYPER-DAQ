...
class HyperDaqApp:
    ...
    def __init__(self):
        ...
        self._window_minutes = float(DEFAULT_WINDOW_VALUE) * WINDOW_UNIT_TO_MINUTES[DEFAULT_WINDOW_UNIT]
        self._notes = load_notes()
        self._save_format = "csv"

        self._group_visible: Dict[str, bool] = {g: True for g in ALL_SENSOR_GROUPS}
        ...
        with dpg.child_window(tag="sidebar", width=self._SIDEBAR_W, height=h, border=True):
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_radio_button(
                    ...
                    callback=lambda s, a: setattr(self, "_save_format", a.lower()),
                )
    ...
    def _on_window_changed(self):
        val = dpg.get_value("win_val") or DEFAULT_WINDOW_VALUE
        unit = dpg.get_value("win_unit") or "Minutes"
        key = "hr" if unit == "Hours" else "min"
        self._window_minutes = max(0.01, float(val)) * WINDOW_UNIT_TO_MINUTES[key]
    ...
    def _poll(self):
        while dpg.is_dearpygui_running():
            try:
                source = load_stream_source()
                with self._lock:
                    cached = self._df
                    last = self._last_seen
                updated = append_new_stream_rows(cached, source, last,
